...
load("Rum_deer/ID_Chr_MolEcol_revised/ROH_ONLY_model_outputs/Female_LBS_model_output_final_ROH_ONLY.RData")
...
summary(female_LBS_model_ROH_ONLY)
...
sols_hu<-as.data.frame(female_LBS_model_ROH_ONLY$Sol)%>%dplyr::select(matches("2):ROH_"))
...
latent_scale_total_var=as.numeric(mean(rowSums(female_LBS_model_ROH_ONLY$VCV))+fixed_effects_var)
...
effect_chr_femaleLBS_hu=ggplot(size_var_explainedhu, aes(x=log(length_Mb), y=log(var_exp), label=CHR)) +
  ...
  labs(x="Log(Chromosome length (Mb))", y="Log(Variance explained by ROHs in hurdle)", tag="C", title = "Female LBS")+
...
sols_pois<-as.data.frame(female_LBS_model_ROH_ONLY$Sol)%>%dplyr::select(matches("1):ROH_"))
...
## total variance in survival (including fixed and random effects)
latent_scale_total_var_poi=as.numeric(mean(rowSums(female_LBS_model_ROH_ONLY$VCV))+fixed_effects_var_p)
...
effect_chr_femaleLBS_pois=ggplot(size_var_explained_pois, aes(x=log(length_Mb), y=log(var_exp), label=CHR)) +
  ...
  theme_classic()+
  theme(text = element_text(size = 16))+
  labs(x="Log(Chromosome length (Mb))", y="Log(Variance explained by ROHs in truncated Poisson)")+
  stat_cor(method = "pearson",size=6, label.y.npc = 0.05, label.x.npc = 0.35)+
  scale_x_continuous(breaks=seq(0, 125, 25))
