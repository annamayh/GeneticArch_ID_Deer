...
load("Rum_deer/ID_Chr_MolEcol_revised/Female_LBS_model_output_final_ROH_ONLY.RData")

var_LBS=sqrt(sd(female_LBS_df$LBS)) ##variance in LBS

#getting standard devaition in ROH per chr 
sd_feLBS=female_LBS_df%>%
  select(41:73)%>% ## getting only the ROH per chromosome 
  apply(2, sd)%>%
  as_tibble()%>%
...
FROH_sum_sol_hu=summary(female_LBS_model_ROH_ONLY)$solutions[4,1]
FROH_sum_hu_upr=summary(female_LBS_model_ROH_ONLY)$solutions[4,2]
FROH_sum_hu_lwr=summary(female_LBS_model_ROH_ONLY)$solutions[4,3]

sols_hu<-as.data.frame(female_LBS_model_ROH_ONLY$Sol)%>%dplyr::select(matches("2):ROH_"))%>% ## taking out sols with FROH included
...
effect_chr_femaleLBS_hu=ggplot(size_var_explainedhu, aes(x=length_Mb, y=var_exp, label=CHR)) +
  geom_point() +
  geom_smooth(method=lm, color="mediumseagreen") +
  geom_text(nudge_x = 3, size=5)+
  theme_classic()+
  theme(text = element_text(size = 16))+
  labs(x="Chromosome length (Mb)", y="Variance explained by ROHs in hurdle", tag="C", title = "Female LBS")+
...
FROH_sum_sol_pois=summary(female_LBS_model_ROH_ONLY)$solutions[3,1]
FROH_sum_pois_upr=summary(female_LBS_model_ROH_ONLY)$solutions[3,2]
FROH_sum_pois_lwr=summary(female_LBS_model_ROH_ONLY)$solutions[3,3]

sols_pois<-as.data.frame(female_LBS_model_ROH_ONLY$Sol)%>%dplyr::select(matches("1):ROH_"))%>% ## taking out sols with FROH included
...
effect_chr_femaleLBS_pois=ggplot(size_var_explained_pois, aes(x=length_Mb, y=var_exp, label=CHR)) +
  geom_point() +
  geom_smooth(method=lm, color="chocolate3") +
  geom_text(nudge_x = 3, size=5)+
  theme_classic()+
  theme(text = element_text(size = 16))+
  labs(x="Chromosome length (Mb)", y="Variance explained by ROHs in truncated Poisson")+
  stat_cor(method = "pearson",size=6)+
  scale_x_continuous(breaks=seq(0, 125, 25))
