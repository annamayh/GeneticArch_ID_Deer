...
load("Rum_deer/ID_Chr_MolEcol_revised/ROH_ONLY_model_outputs/Juvenile_survival_model_output_ROH_ONLY.RData")
...
effect_chr_js_main=ggplot(size_var_explained, aes(x=log(length_Mb), y=log(var_exp), label=CHR)) +
  ...
  labs(x="Log(Chromosome length (Mb))", y="Log(Variance explained by ROHs)", title="Juvenile Survival", tag="B")+
